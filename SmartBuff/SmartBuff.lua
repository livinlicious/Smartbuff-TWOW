...
function SMARTBUFF_OnEvent(event)
    -- SMARTBUFF_AddMsgWarn("SB Debug: OnEvent " .. event);
...
function SMARTBUFF_AddMsg(msg, force)
  if (DEFAULT_CHAT_FRAME) then
    if (not SMARTBUFF_Options or force or not SMARTBUFF_Options.ToggleMsgNormal) then
...
function SMARTBUFF_AddMsgErr(msg, force)
  if (DEFAULT_CHAT_FRAME) then
    if (not SMARTBUFF_Options or force or not SMARTBUFF_Options.ToggleMsgError) then
...
function SMARTBUFF_AddMsgWarn(msg, force)
  if (DEFAULT_CHAT_FRAME) then
    if (not SMARTBUFF_Options or force or not SMARTBUFF_Options.ToggleMsgWarning) then
...
function SMARTBUFF_SetBuffs()
  ...
  -- Weapon buffs moved to LAST so they don't interrupt other buffs with GCD
  -- Using explicit numeric loop instead of generic for
  for j = 1, table.getn(SMARTBUFF_WEAPON) do
    if (SMARTBUFF_WEAPON[j]) then
       local bName = SMARTBUFF_WEAPON[j][1];
       n = SMARTBUFF_SetBuff(SMARTBUFF_WEAPON[j], n);
    end
  end
...
function SMARTBUFF_SetBuff(buff, i)
  ...
  if (cBuffs[i].IDS) then
    ...
    -- For inventory items (weapon buffs), try to find the item
    local bag, slot, count, texture = SMARTBUFF_FindReagent(cBuffs[i].BuffS);
    -- Don't remove the buff from cBuffs if item not found during initialization
    -- The item might not be loaded yet on game start
    -- Instead, just use a default icon and check for the item later when buffing
    if (count == 0) then
      -- Item not found, but still add the buff to cBuffs
      cBuffs[i].IconS = "Interface\\Icons\\INV_Misc_QuestionMark";
    else
  ...
  if (SMARTBUFF_Buffs[ct][name] == nil) then
    ...
    -- For weapon buffs, default MH to true so they work on first install
...
function SMARTBUFF_PreCheck(mode, force)
  ...
  local timeLeft = SMARTBUFF_Options.AutoTimer - (GetTime() - tLastCheck);
  if (mode == 1 and force == nil and timeLeft > 0) then
    return false;
  else
    tLastCheck = GetTime();
  end
          ...
          if (cBuffs[i].IDS) then _, cd = GetSpellCooldown(cBuffs[i].IDS, SMARTBUFF_BOOK_TYPE_SPELL); end
            ...
            if ((cBuffs[i].Type == SMARTBUFF_CONST_GROUP and ((SMARTBUFF_Buffs[ct][buffnS][uc] and (uct == SMARTBUFF_HUMANOID or (uc == "DRUID" and uct == SMARTBUFF_BEAST))) or (SMARTBUFF_Buffs[ct][buffnS]["HPET"] and uct == SMARTBUFF_BEAST and uc ~= "DRUID") or (SMARTBUFF_Buffs[ct][buffnS]["WPET"] and uct == SMARTBUFF_DEMON and ucf ~= SMARTBUFF_DEMONTYPE)))
              ...
              elseif (cBuffs[i].Type == SMARTBUFF_CONST_WEAPON or cBuffs[i].Type == SMARTBUFF_CONST_INV) then
                -- PRIORITY CHECK: Skip weapon buffs if ANY higher priority buff needs casting
                -- Check all buffs that come BEFORE this weapon buff in the list
                local hasHigherPriorityBuffNeeded = false;
                for checkIdx = 1, i - 1 do
                  if (cBuffs[checkIdx] and cBuffs[checkIdx].BuffS and cBuffs[checkIdx].Type ~= SMARTBUFF_CONST_WEAPON and cBuffs[checkIdx].Type ~= SMARTBUFF_CONST_INV) then
                    -- This is a non-weapon buff with higher priority (earlier in list)
                ...
                if (hasHigherPriorityBuffNeeded) then
                  -- Skip this weapon buff, higher priority buffs need to be cast first
                  buff = nil;
                else
                  -- No higher priority buffs needed, proceed with weapon buff check
                  local bMh = false;
                  local bOh = false;
                  local tMh = -1;
                  local tOh = -1;
                ...
                if (SMARTBUFF_Buffs[ct][buffnS].MH or cBuffs[i].Type == SMARTBUFF_CONST_WEAPON) then
                  local canApply = SMARTBUFF_CanApplyWeaponBuff(buffnS, 16);

                  if (mainH and canApply) then
					          if (bMh) then
                ...
                end -- Close the else block for weapon buff priority check
...
function SMARTBUFF_CanApplyWeaponBuff(buff, slot)
  ...
  -- Robust GetItemInfo extraction handling possible minLevel field variations
  local a1, a2, a3, a4, a5, a6, a7, a8, a9 = GetItemInfo(itemCode);

  -- Determine ItemType and SubType (usually localized) and EquipLoc (usually global)
  -- Standard 1.12: a5=Type, a6=SubType, a8=EquipLoc
  -- Turtle/Classic: a6=Type, a7=SubType, a9=EquipLoc

  local itemType, itemSubType, equipLoc;

  -- Try to find INVTYPE in a8 or a9
  if (a8 and string.find(a8, "INVTYPE_")) then
      equipLoc = a8;
      itemType = a5;
      itemSubType = a6;
  elseif (a9 and string.find(a9, "INVTYPE_")) then
      equipLoc = a9;
      itemType = a6;
      itemSubType = a7;
  else
      -- Fallbacks if checking standard 1.12
      itemType = a5;
      itemSubType = a6;
      equipLoc = a8;
  end
  ...
  -- Original check: exact subType match against list
  if (itemSubType and cWeaponTypes) then
  ...
  -- Force allow for OILS (Standard List) if it looks like a weapon
  -- This bypasses the specific subtype list if it failed
  if (not b and cWeaponTypes == SMARTBUFF_WEAPON_STANDARD) then
     if (equipLoc) then
       if (equipLoc == "INVTYPE_WEAPON" or equipLoc == "INVTYPE_2HWEAPON" or equipLoc == "INVTYPE_WEAPONMAINHAND" or equipLoc == "INVTYPE_WEAPONOFFHAND") then
         b = true;
       end
     end
     -- Extra backup: Check if Type is "Weapon" (localized in English)
     if (not b and itemType and (itemType == "Weapon" or itemType == "Weapons")) then
        b = true;
     end
  end
		...
		local itemName = GetContainerItemLink(bag, slot);
			if itemName then
				if string.find(itemName, "%[" .. reagent .. "%]") then
                    ...
                    -- If we found the item and it has a valid count, return 1 (exists)
                    -- Handle negative counts from Turtle WoW bug
	...
	for bag = 0, NUM_BAG_FRAMES do
		...
		local itemName = GetContainerItemLink(bag, slot);
			if itemName then
				if string.find(itemName, "%[" .. reagent .. "%]") then
					local texture, count = GetContainerItemInfo(bag, slot);
          -- Fix negative count issue - if item exists, return positive count
          if (count and count ~= 0) then
            if (count < 0) then count = math.abs(count); end
...
function SMARTBUFF_Options_Init()
  ...
  SMARTBUFF_AddMsg("/sb menu - " .. SMARTBUFF_OFT_MENU, true);
	...
	if (SMARTBUFF_Options.FirstStart ~= SMARTBUFF_VERSION) then
	  ...
	  -- else -- Removed else to force SetBuffs
	end
    SMARTBUFF_SetBuffs(); -- Always run this
  SMARTBUFF_SetUnits();
end
...
function SMARTBUFF_SetCheckButtonBuffs()
    ...
    if (cBuffs[buffIdx].IDS ~= nil) then
      shouldShow = true;
    elseif (cBuffs[buffIdx].Type == SMARTBUFF_CONST_INV) then
      -- For weapon buffs, only show if item is in bags
      local cr = SMARTBUFF_CountReagent(cBuffs[buffIdx].BuffS);
      if (cr > 0) then
        shouldShow = true;
      end
    end
    ...
    if (shouldShow) then
      ...
      uiSlotToBuffIdx[uiSlot] = buffIdx;
        ...
        getglobal(objG:GetName().."Text"):SetText(cBuffs[buffIdx].BuffS .. " / " .. SMARTBUFF_MSG_GROUP);
	      objG:SetChecked(SMARTBUFF_Buffs[currentTemplate][cBuffs[buffIdx].BuffS].EnableG);
	      objG:Show();
	    else
	      if (objG) then objG:Hide(); end
	      getglobal(objS:GetName().."Text"):SetText(cBuffs[buffIdx].BuffS);
	    end
      objS:SetChecked(SMARTBUFF_Buffs[currentTemplate][cBuffs[buffIdx].BuffS].EnableS);
      objS:Show();
	...
	-- Second pass: hide remaining UI slots
	while (uiSlot <= maxCheckButtons) do
    objS = getglobal("SmartBuffOptionsFrame_cbBuffS"..uiSlot);
    objG = getglobal("SmartBuffOptionsFrame_cbBuffG"..uiSlot);
